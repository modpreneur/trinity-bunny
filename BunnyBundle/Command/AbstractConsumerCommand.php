 ...
 * @package Trinity\Bundle\BunnyBundle\Command
 */
abstract class AbstractConsumerCommand extends ContainerAwareCommand
{
    /** @var int */
    protected $t = null;
    ...
    protected function initialize(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!is_numeric($max)) {
            throw new InvalidArgumentException('Parameter \'max-messages\' must be numeric value.');
        }
        if ($max < 0) {
            throw new InvalidArgumentException('Parameter \'max-messages\' must be greater than 10.');
        }
    ...
    protected function configure()
    {
        ...
        $this->setName('necktie:rabbit:baseConsumer')->setDescription('Start waiting for messages.')->addArgument(
                'consumer',
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $ch->consume(
            function (Message $msg, Channel $ch, Client $c) use (&$t, &$count, $input, $output) {
